...
const FURNITURE_CATEGORY_ID = "69a1f299c92e1e7aca7f27fb";

function FurniturePageContent() {
    const router = useRouter();
    const searchParams = useSearchParams();
    const subcategoryId = searchParams.get("subcategory");
    const { subcategories } = useCachedSubcategories(FURNITURE_CATEGORY_ID);
                    ...
                    <div className="mb-8 animate-fadeInUp">
                        <div className="flex flex-wrap justify-center gap-2 sm:gap-3 overflow-x-auto pb-2 scrollbar-thin">
                            ...
                            {subcategories.map((sub) => (
                                <Link
                                    key={sub._id}
                                    href={`/furniture?subcategory=${encodeURIComponent(sub._id)}`}
                                    className={`flex-shrink-0 px-4 py-2 rounded-full border text-sm font-medium transition-colors shadow-sm ${
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-16">
                        <div className="max-w-md mx-auto">
                            ...
                            <p className="text-gray-600">We couldn't find any furniture products at the moment. Please check back later.</p>
                        </div>
                    </div>
                ) : (
                    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
...
function FurniturePageFallback() {
    return (
...
export default function FurniturePage() {
    return (
        <Suspense fallback={<FurniturePageFallback />}>

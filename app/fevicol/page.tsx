...
const FEVICOL_CATEGORY_ID = "69a1f28ec92e1e7aca7f27f5";

function FevicolPageContent() {
    const router = useRouter();
    const searchParams = useSearchParams();
    const subcategoryId = searchParams.get("subcategory");
    const { subcategories } = useCachedSubcategories(FEVICOL_CATEGORY_ID);
                ...
                <div className="text-center mb-16 relative overflow-hidden">
                    ...
                    <div className="relative animate-fadeInDown">
                        ...
                        <p className="text-lg text-gray-600 font-medium animate-fadeInUp delay-200">
                            Strong adhesive solutions for professional and home use
                        </p>
                    ...
                    <div className="mb-8 animate-fadeInUp">
                        <div className="flex flex-wrap justify-center gap-2 sm:gap-3 overflow-x-auto pb-2 scrollbar-thin">
                            ...
                            {subcategories.map((sub) => (
                                <Link
                                    key={sub._id}
                                    href={`/fevicol?subcategory=${encodeURIComponent(sub._id)}`}
                                    className={`flex-shrink-0 px-4 py-2 rounded-full border text-sm font-medium transition-colors shadow-sm ${
...
export default function FevicolPage() {
    return (
        <Suspense fallback={<FevicolPageFallback />}>
            <FevicolPageContent />
        </Suspense>
    );
}
